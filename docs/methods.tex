...
% Regenerate with the notebooks in notebooks/ and notebooks/arxiv/ if the
% pipeline is re-run.
%
% PENDING RE-EXTRACTION under registry v0.4.0. The registry description below is
% v0.4.0 (current); the *counts* are still from the tables built under v0.1.0
% (S2ORC) and v0.2.0 (arXiv). v0.4.0 changed matching -- one shared bare-code
% rule, case-insensitive vendor-qualified aliases, the 1080Ti/RTX-8000 forms --
% so a full single-version re-scan of both corpora is underway. Numbers to
% re-read from the v0.4.0 tables when it lands, and nothing else:
%   - the candidate/passage/mention funnel counts in "Candidate passage
%     generation" and "LLM extraction and validation";
%   - the tier-B alias count and the alias totals quoted as scan-time figures;
%   - the canonicalization rates and unresolved-bucket shares in "Normalization";
%   - the pre-release-mention rate in "Validity checks";
%   - and the paragraph on cross-version divergence, which the re-scan retires
%     outright (both corpora will share one registry version).
% Everything descriptive -- rules, conventions, thresholds -- is already v0.4.0.
%
% MIXED INVENTORY: `inventory/parts/` is not namespaced by registry version, and
% an 8-shard pilot re-scan under v0.3.1 overwrote those parts in place. So
% `is_candidate` reflects v0.3.1 for 8 of 214 S2ORC shards and v0.1.0 for the
% rest: 399,690 candidate papers against 399,841 before, a 0.04% difference that
% moves only the funnel's candidate row. A full stage-1 re-scan makes it uniform.
%
...
scraped --- the context-gate vocabularies, the generic terms, and per-id
overrides that replace a generated entry should a scrape ever go wrong.

Because both generators write into one alias namespace, the rules that decide
alias \emph{tier} are stated once and shared
(\texttt{scripts/alias\_rules.py}) rather than per generator. A bare short code
receives a gated alias unless (i) the code carries a named non-hardware meaning,
or (ii) another entry already claims that exact string. Clause (i) is an
enumerated list, each item recording the collision that earns it a place
(\texttt{T4} thyroxine and the thoracic vertebra, \texttt{L2} regularization and
cache, \texttt{H20} water, \texttt{MI6} the agency, \texttt{A10} the aircraft,
\texttt{C60} fullerene), plus a general clause denying every all-digit code,
since a bare \texttt{7210} or \texttt{910} is a quantity in prose and no context
vocabulary distinguishes the two. Clause (ii) covers genuine cross-vendor
collisions: Intel's Arc~Pro~A40 and NVIDIA's A40 are different devices with one
code, as are the Radeon~Pro~5500~XT and the RX~5500~XT, so the bare form
identifies neither and each keeps its qualified spelling. Denied models remain
reachable through a vendor- or line-qualified alias (``NVIDIA L4'', ``Tesla T4'',
``Xeon Phi 7210''). A length threshold was considered as the rule and rejected:
it states more simply but discards \texttt{K80}, \texttt{K20}, \texttt{K40},
\texttt{P40}, \texttt{A30}, \texttt{A40} and \texttt{L40}, all real devices
with substantial paper support. Conversely, an alias whose leading token is a
vendor name is compiled case-insensitively, overriding the general rule that an
all-uppercase pattern matches case-sensitively; without this, ``Nvidia L4''
--- the ordinary spelling --- did not match, a directional recall loss that fell
on precisely the vendor-qualified-only export-control parts (L4, A800, HGX~H20).

Registry v0.4.0 contains $1{,}226$ entries: $1{,}217$ specific models and 9
generic terms (\texttt{GPU},
\texttt{CUDA}, \texttt{TPU}, \ldots, first-class entries so that generic-only
reporting is measurable), spanning 22 manufacturers. The largest are NVIDIA 556,
AMD 542, Intel 43, Apple 18 and Google 10, then a long tail of eight or fewer
...
datacenter 130, cloud 22. Every specific model is generated, as are the two
vendor brand-trigger entries: their aliases are the product-line names that lead
model names in the tables, kept when they name at least five models and screened
against a vendor-neutral list of words that are a different product category or
ordinary English. Hand-listing them had given NVIDIA three brands (GeForce,
Quadro, Tesla) and AMD one (Radeon), while the same tables carry five AMD lines
(Radeon, FirePro, FireGL, FireMV, Instinct) --- an asymmetry in candidate recall
that favoured the vendor whose share the paper reports. The hand-maintained file
holds only the three gate vocabularies and the seven generic terms, neither of
which any hardware table supplies; it names no device.
...
names (Pascal, Hopper, CDNA, \ldots) likewise have no entry of their own: an
architecture is a generation rather than a device, has no per-device
specification, and is already carried as a metadata column on every model entry,
so an architecture-grain mention resolves to its vendor and is counted as
brand-level reporting. This costs nothing at model level --- $278{,}978$ S2ORC and
$317{,}955$ arXiv model-kind used-mentions are identical with and without those
entries --- because an architecture was never model-specific.
...
current registry's specifications (v0.3.1, which additionally splits Apple
...
literal cores of every alias in the extraction-time registry ($2{,}178$ in
v0.4.0) prefilters paragraphs; a hit must fall on
...
one alias. Note that the separator is zero-or-more, so a closed-up
\texttt{TeslaV100} matches by design. Case sensitivity is derived from the
pattern: all-uppercase aliases (\texttt{GPU}, \texttt{V100}, \texttt{K80}) match
case-sensitively and mixed-case ones (\texttt{Tesla V100}) case-insensitively,
except that a vendor-led pattern is always case-insensitive (see the registry
subsection). Overlapping matches are resolved by longest span, so
\texttt{RTX 3090 Ti} beats \texttt{RTX 3090} without hand-authored lookaheads;
two \emph{equal-length} spans from different entries are both retained and the
disambiguation is left to the LLM and to canonicalization. (iii) Ambiguous
aliases --- short codes whose strings collide with non-hardware usage
(\texttt{K80} the antibody, \texttt{M2} the money supply, brand words that are
ordinary English) --- are marked tier~B ($318$ of v0.4.0's $2{,}178$ aliases;
$236$ of $1{,}862$ in the registry that scanned arXiv) and count only if a
context term from the relevant gate vocabulary (\texttt{gpu}: 21 terms;
\texttt{tpu}: 5; \texttt{apple}: 8) appears within the matched paragraph or the
adjacent $250$ characters of either neighbouring paragraph. The gate is
position-independent within that window rather than a $\pm250$-character
neighbourhood of the match itself, which makes it permissive for a long
paragraph; that is the intended direction. A paper-level rescue then un-gates
tier~B matches in papers carrying at least one ungated model hit anywhere, and we
report the share of tier-B used-mentions that survive only by rescue, together
with a local-gate-only series as robustness. Gating errs permissive by design: it
exists to bound candidate volume, not to be precise, because precision is the
LLM's job.
...
generic-only papers (\texttt{GPU}/\texttt{CUDA} with no model named), truncating
in document order, with a
...
the registry hint surfaces, and the passage text. Passing the hint surfaces
imports the registry's attention into the prompt, which is why the worked
examples include a negative and why the schema provides two explicit ways to
reject; the \texttt{no\_mention} rates below ($5.5\%$ of S2ORC and $9.1\%$ of
arXiv rows) are the direct evidence that the model does overrule the hint rather
than ratify it. Prompt text is versioned
...
\textbf{External validation of candidate generation.} The checks above are
structural; they do not say whether the registry finds the hardware a human
annotator would mark. We therefore run the matcher, unchanged, over
% swap in the Related Work citation key for SH-NER when merging
\textbf{SH-NER} (Anjum et al., 2025), an independently constructed corpus of
manually annotated infrastructure entities in NLP papers, and treat its
\texttt{Hardware-device} spans as ground truth. On the test split ($1{,}205$
sentences, $174$ annotated spans, $60$ papers), $86.8\%$ of spans are recovered
from local context alone and $89.1\%$ once the paper-level rescue is applied,
with $82.8\%$ resolving to a specific model entry rather than to a generic or
brand term; $96.8\%$ of sentences carrying an annotated span would generate a
candidate passage. Every miss is classified rather than merely counted: $11$ of
$19$ are CPUs, which SH-NER's \texttt{Hardware-device} class includes and our
registry deliberately excludes; $5$ are rented cloud instance types
(\texttt{p3.16xlarge}, \texttt{g4dn.xlarge}), which name a bundle rather than a
device and whose resolution would require an instance-to-device table no source
we parse publishes; and $3$ are a genuinely ambiguous ``Titan X'', a memory size,
and a further CPU. Excluding the classes we do not claim to cover, span recall is
$95.7\%$. This is a conservative reading of the pipeline: SH-NER labels single
sentences, whereas the scan sees a paragraph and its neighbours. Precision is
deliberately not reported against these labels, because the registry matches CPUs
and FPGAs on purpose and forwards them to the LLM, so each such match would score
as a false positive against a definition we do not share. Two registry gaps this
exercise surfaced --- bare \texttt{1080Ti} and \texttt{RTX8000} --- were fixed
before the final scan and pinned as test fixtures.
...
Upstream of that, each Wikipedia source is pinned by revision id, so
\texttt{release\_source} and \texttt{spec\_source} resolve to immutable
permalinks rather than to pages that mutate. The parsed source snapshots
themselves are archived with the code, and each generator records the SHA-256 of
every input it read, so both generators re-run from a clean checkout and reproduce
their emitted registry files exactly. Because matching is decided by the hand file
\emph{and} the generated files together, a digest over all of them is committed
and tested, so no re-scrape can change an alias without changing the version that
names the outputs.
