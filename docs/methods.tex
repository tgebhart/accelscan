...
% (registry v0.2.0; extraction 2026-07/08). Regenerate with the notebooks in
...
page, and the Tensor Processing Unit page, collapsing per-variant rows (memory
size, form factor, edition) into canonical models and merging duplicate entries
across pages by taking the earliest launch date, the union of aliases, and the
element-wise maximum of specifications. Only rows with a parseable launch date
...
Registry v0.2.0 contains $1{,}139$ entries: $1{,}117$ specific models, 13
architectures (Fermi, Kepler, \ldots), and 9 generic terms (\texttt{GPU},
\texttt{CUDA}, \texttt{TPU}, \ldots, first-class entries so that generic-only
reporting is measurable). By manufacturer: NVIDIA 562, AMD 544, Google 10,
Intel 6, Apple 4, Cerebras 3, Huawei 2, Amazon 2, Graphcore 1. By market
segment: consumer 908, workstation 130, datacenter 62, cloud 11, embedded 6.
Release dates span 2000--2026 and are populated for all $1{,}117$ models, since
the frontier-lag and half-life estimands depend on them. Per-device
specification coverage across models is: memory $97.5\%$, FP32 $63.5\%$, FP64
$46.6\%$, dense FP16 tensor throughput $5.8\%$ (defined only for post-Volta
parts), TDP $68.9\%$. Missing specifications remain null and are excluded
...
sparsity-doubled marketing figures are excluded, and TF32 tensor throughput is
never recorded in the FP32 column.
...
2026-07-28). The bump added per-device specifications to existing entries and 12
new models, removing none, so the matching surface is all but identical across
corpora. Both capacity tables use v0.2.0 specifications; because specification
additions do not change matching, they require re-running only stages 3.5 and
later, never the scan or the LLM.
...
literal cores of all $1{,}862$ aliases prefilters paragraphs; a hit must fall on
...
tier~B ($236$ of $1{,}862$ aliases) and count only if a context term from the
